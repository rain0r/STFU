    ...
    extends SimpleTest
{
        ...
        byte[]  out)
    {
        try
        {
            Cipher wrapper = Cipher.getInstance("DESedeWrap", "BC2");
        ...
        byte[]      output)
    {
        ...
        try
        {
            keyGen = KeyGenerator.getInstance("DESEDE", "BC2");
            keyGen.init(strength, rand);

            key = keyGen.generateKey();

            in = Cipher.getInstance("DESEDE/ECB/PKCS7Padding", "BC2");
            out = Cipher.getInstance("DESEDE/ECB/PKCS7Padding", "BC2");
        ...
        try
        {
            SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DESede", "BC2");
