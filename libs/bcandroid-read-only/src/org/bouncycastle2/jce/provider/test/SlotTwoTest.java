    ...
    extends SimpleTest
{
    ...
    public void performTest() 
        throws Exception
    {
        Security.removeProvider("BC2");
        Security.insertProviderAt(new BouncyCastleProvider(), 2);

        KeyGenerator keyGen = KeyGenerator.getInstance("DESede", "BC2");
    ...
    private void testDesEde(
        ...
        throws Exception
    {
        Cipher encrypt = Cipher.getInstance("DESede/" + mode + "/" + padding, "BC2");
        Cipher decrypt = Cipher.getInstance("DESede/" + mode + "/" + padding);
        
        if (!decrypt.getProvider().getName().equals("BC2"))
        {
